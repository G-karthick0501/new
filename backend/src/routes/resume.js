...
router.post("/analyze-initial", auth, upload.fields([
  ...
  try {
    ...
    const jdText = await PdfParse(jd.buffer);
    ...
    formData.append('job_description', jdText.text);

    // Call Resume Analyzer Service endpoint
    const aiResponse = await axios.post('http://localhost:8000/analyze-resume', formData, {
    ...
    if (error.code === 'ECONNREFUSED') {
      return res.status(503).json({ 
        ...
        details: "Make sure the AI service is running on http://localhost:8003",
...
router.post("/generate-optimized", auth, async (req, res) => {
  try {
    ...
    // Call Resume Analyzer Service endpoint
    const aiResponse = await axios.post('http://localhost:8003/optimize-resume', formData, {
      headers: formData.getHeaders(),
      timeout: 120000
    });
...
router.post("/download-pdf", auth, async (req, res) => {
  try {
    ...
    const response = await axios.post(
      'http://localhost:8003/optimize-resume-pdf',
      formData,
      {
        headers: formData.getHeaders(),
    ...
    if (error.code === 'ECONNREFUSED') {
      return res.status(503).json({ 
        msg: "AI service unavailable",
        hint: "Ensure Python service is running on http://localhost:8003"
      });
    }
...
router.post("/download-latex-pdf", auth, async (req, res) => {
  try {
    ...
    const response = await axios.post(
      'http://localhost:8003/optimize-resume-latex-pdf',
      formData,
      {
        headers: formData.getHeaders(),
